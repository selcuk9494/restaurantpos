<?php
/* Template Name: Entegratörler */

get_header(); ?>

<div class="bg-light py-5">
    <div class="container">
        <h1 class="fw-bold text-center">Entegrasyon Uyumlu Yazılımlar</h1>
        <p class="text-center text-muted">Sistemlerimizle tam uyumlu çalışan entegratör iş ortaklarımız.</p>
    </div>
</div>
...
<section class="py-5">
    <div class="container">
        <div class="row row-cols-2 row-cols-md-4 row-cols-lg-6 g-4 justify-content-center align-items-center">
            ...
            $int_dir_path = $base_img_path . $final_folder_name . '/';
            $int_dir_url = $base_img_url . $final_folder_name . '/';
            
            // Klasördeki resim dosyalarını bul (Scandir yöntemi - En uyumlu)
            $images = array();
            
            if (is_dir($int_dir_path)) {
                $files = scandir($int_dir_path);
                if ($files !== false) {
                    $allowed_exts = array('png', 'jpg', 'jpeg', 'webp');
                    foreach ($files as $file) {
                        $ext = strtolower(pathinfo($file, PATHINFO_EXTENSION));
                        if (in_array($ext, $allowed_exts)) {
                            $images[] = $int_dir_path . $file;
                        }
                    }
                }
            } else {
                // Debug Bilgisi (Yönetici ise göster)
                if (current_user_can('administrator')) {
                    echo '<div class="alert alert-warning col-12">';
                    echo '<strong>Hata:</strong> Klasör bulunamadı.<br>';
                    echo 'Aranan Yol: ' . $int_dir_path . '<br>';
            ...
            if (!empty($images)) {
                foreach ($images as $image_path) {
                    ...
                    $image_url = $int_dir_url . $image_name;
                    ...
                    // Bazı özel temizlemeler (removebg-preview vb.)
                    $display_name = str_replace('Removebg Preview', '', $display_name);
                    $display_name = str_replace('Guncel', '', $display_name);
                ...
                echo '<div class="col-12 text-center"><p class="text-muted">Henüz entegratör görseli eklenmemiş.</p></div>';
            }
            ?>
        </div>
    </div>
